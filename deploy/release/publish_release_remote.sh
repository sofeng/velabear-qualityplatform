...
USER_NAME=""
PASSWORD=""
REMOTE_RELEASE_ROOT="/AIOps/releases/testhub-platform"
...
usage() {
  cat <<'EOF'
Usage:
  publish_release_remote.sh --release-dir <dir> --remote-host <host> --user <user> --password <password>
...
required_files=(
  ...
  "$RESOLVED_RELEASE_DIR/runtime/deploy/release/remote_apply_release.sh"
  "$RESOLVED_RELEASE_DIR/runtime/deploy/release/remote_smoke_verify.sh"
  "$RESOLVED_RELEASE_DIR/runtime/deploy/release/remote_rollback_release.sh"
)
...
if [ "$AUTO_APPLY" = "1" ]; then
  ...
  sshpass -p "$PASSWORD" ssh "${SSH_OPTS[@]}" "$USER_NAME@$REMOTE_HOST" \
    "bash '$REMOTE_RELEASE_DIR/runtime/deploy/release/remote_apply_release.sh' --runtime-dir '$RUNTIME_DIR' --release-dir '$REMOTE_RELEASE_DIR' $smoke_flag"
fi

printf 'Remote release directory: %s\n' "$REMOTE_RELEASE_DIR"
