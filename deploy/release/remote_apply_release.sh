...
usage() {
  cat <<'EOF'
Usage:
  remote_apply_release.sh --runtime-dir <dir> --release-dir <dir> [--run-smoke]
EOF
}
...
cp "$COMPOSE_FILE" "$HISTORY_DIR/docker-compose.offline.yml.before"
cp "$RELEASE_ENV" "$HISTORY_DIR/release.env"
if [ -f "$RELEASE_DIR/runtime/deploy/docker/docker-compose.offline.yml" ]; then
  cp "$RELEASE_DIR/runtime/deploy/docker/docker-compose.offline.yml" "$COMPOSE_FILE"
fi
cp "$RELEASE_DIR/runtime/deploy/release/remote_apply_release.sh" "$RUNTIME_RELEASE_DIR/remote_apply_release.sh"
cp "$RELEASE_DIR/runtime/deploy/release/remote_smoke_verify.sh" "$RUNTIME_RELEASE_DIR/remote_smoke_verify.sh"
cp "$RELEASE_DIR/runtime/deploy/release/remote_rollback_release.sh" "$RUNTIME_RELEASE_DIR/remote_rollback_release.sh"
...
if [ "$found_tar" -ne 1 ]; then
  printf 'No image tar files found in %s/images\n' "$RELEASE_DIR" >&2
  exit 1
fi
...
log "Release applied successfully: $RELEASE_TAG"
