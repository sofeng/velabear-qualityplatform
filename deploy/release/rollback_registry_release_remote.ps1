...
$askpass = Join-Path $env:TEMP 'codex-registry-rollback-askpass.cmd'
...
try {
    ...
    $registryEndpointArg = if ($RegistryEndpoint) { "--registry-endpoint $RegistryEndpoint" } else { '' }
    $registryNamespaceArg = if ($RegistryNamespace) { "--registry-namespace $RegistryNamespace" } else { '' }
    $smokeArg = if ($RunSmoke) { '--run-smoke' } else { '' }
    $command = "bash $RuntimeDir/deploy/release/remote_rollback_registry_release.sh --runtime-dir $RuntimeDir $imageTagArg $historyArg --components $componentsCsv $registryEndpointArg $registryNamespaceArg $smokeArg"
    ssh -p $RemotePort -o StrictHostKeyChecking=no "$User@$RemoteHost" $command
    if ($LASTEXITCODE -ne 0) { throw 'Remote registry rollback failed.' }
}
finally {
