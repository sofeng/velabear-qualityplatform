...
usage() {
  cat <<'EOF'
Usage:
  rollback_registry_release_remote.sh --remote-host <host> --user <user> --password <password>
    [--remote-port 22] [--runtime-dir <dir>] [--image-tag <tag>] [--history-dir <dir>]
    [--components backend,frontend] [--registry-endpoint <host:port>] [--registry-namespace <path>] [--run-smoke]
EOF
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --registry-endpoint) REGISTRY_ENDPOINT="$2"; shift 2 ;;
...
image_tag_arg=""
history_dir_arg=""
registry_endpoint_arg=""
registry_namespace_arg=""
run_smoke_arg=""
...
[ -n "$REGISTRY_ENDPOINT" ] && registry_endpoint_arg="--registry-endpoint '$REGISTRY_ENDPOINT'"
[ -n "$REGISTRY_NAMESPACE" ] && registry_namespace_arg="--registry-namespace '$REGISTRY_NAMESPACE'"
[ "$RUN_SMOKE" = "1" ] && run_smoke_arg="--run-smoke"

sshpass -p "$PASSWORD" ssh "${SSH_OPTS[@]}" "$USER_NAME@$REMOTE_HOST" \
  "bash '$RUNTIME_DIR/deploy/release/remote_rollback_registry_release.sh' --runtime-dir '$RUNTIME_DIR' $image_tag_arg $history_dir_arg --components '$COMPONENTS' $registry_endpoint_arg $registry_namespace_arg $run_smoke_arg"
