    ...
    <div v-loading="loading" class="card-container defect-form-card">
      <el-form ref="formRef" :model="form" :rules="rules" label-width="100px">
        <el-tabs v-model="activeTab" class="defect-form-tabs">
                        ...
                        >
                          ...
                          <div v-loading="moduleCategoryLoading" class="module-selector__dropdown">
                            ...
                            <el-tree
                              ...
                              node-key="id"
                              :default-expanded-keys="moduleTreeDefaultExpandedKeys"
                            ...
                            >
                              <template #default="{ data }">
                                <div class="module-selector__tree-node" :title="data.fullPath || data.label">
                                  <span class="module-selector__tree-label">{{ data.label }}</span>
                                </div>
                              </template>
                            </el-tree>
                            <el-empty
          ...
          <el-tab-pane label="缺陷处理" name="process">
            <div class="tab-panel">
              <div class="tab-card defect-section-card">
                <div class="tab-card__title">处理信息</div>
                <el-row :gutter="20">
                  <el-col :xs="24" :md="8">
          ...
          <el-tab-pane label="缺陷分析" name="analysis">
            <div class="tab-panel">
              <div class="tab-card defect-section-card">
                <div class="tab-card__title">缺陷分析</div>
                <el-row :gutter="20">
                  <el-col :xs="24" :md="12">
...
import { defectStatusOptions } from '@/utils/defectStatus'
...
const editableDefectTabs = new Set(['basic', 'relations', 'process', 'analysis'])
...
const moduleTreeDefaultExpandedKeys = computed(() =>
  moduleCategoryTree.value.map((item) => item.id).filter((id) => id !== undefined && id !== null)
)
...
const showSubmitAction = computed(() => !isEdit.value || editableDefectTabs.has(activeTab.value))
...
const manualWorkspaceQuery = computed(() => {
  ...
  if (tab === 'version-defects' && testpointId) {
    query.defect_testpoint_id = testpointId
  }
...
const buildSourceDescriptionLines = (source = {}) => {
  ...
  if (source.frontendOwner) {
    lines.push(`前端开发：${source.frontendOwner}`)
  }
  if (source.backendOwner) {
    lines.push(`后端开发：${source.backendOwner}`)
  }
...
const goBackToList = () => {
  ...
  if (currentTab === 'version-defects' && getQueryValue(route.query.testpoint_id)) {
    query.defect_testpoint_id = String(getQueryValue(route.query.testpoint_id))
  }
...
const ensureJiraRequirementOption = (option = {}) => {
  ...
  const existingOption = optionMap.get(issueKey) || {}
  optionMap.set(issueKey, {
    issue_key: issueKey,
    summary: String(option.summary || existingOption.summary || '').trim(),
    version: String(option.version || existingOption.version || '').trim(),
  })
...
const loadDefect = async () => {
  ...
  loading.value = true
  try {
    const response = await getDefectDetail(route.params.id)
...
const handleCommentSubmit = async () => {
  ...
  try {
    ...
    await addDefectComment(route.params.id, preparedCommentContent)
...
const handleCommentEditSubmit = async (comment) => {
  ...
  try {
    ...
    await updateDefectComment(route.params.id, comment.id, preparedCommentContent)
    cancelEditingComment()
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (error?.response?.data) {
      ...
      ElMessage.error(errorMessage || '保存缺陷失败')
