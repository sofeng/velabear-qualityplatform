    ...
    <div class="tab-panel defect-list-panel">
      <div class="tab-toolbar">
        <el-form :inline="true" :model="filters" class="search-form defect-search-form">
          ...
          <el-form-item label="关键字">
            <el-input
              ...
              clearable
              placeholder="搜索标题、描述、问题原因、问题根因或需求编号"
              style="width: 220px"
              @keyup.enter="handleSearch"
            />
          </el-form-item>
        ...
        <div class="tab-toolbar-actions defect-toolbar-actions">
          <div class="defect-toolbar-buttons">
            <TableColumnSettings
              :table-ref="defectTableRef"
              storage-key="manual-testcases.defects"
            />
      ...
      >
        ...
        <el-table-column
          prop="priority"
          label="优先级"
          :width="defectColumnWidths.priority"
          sortable
        ...
        <el-table-column
          prop="problem_reason"
          label="问题原因"
          :width="defectColumnWidths.problem_reason"
          show-overflow-tooltip
          sortable
        ...
        <el-table-column
          prop="root_cause"
          label="问题根因"
          :width="defectColumnWidths.root_cause"
          show-overflow-tooltip
          sortable
        ...
        <el-table-column
          label="需求编号"
          :width="defectColumnWidths.requirement_id"
          sortable
        ...
        <el-table-column
          ...
          label="前端开发"
          :width="defectColumnWidths.frontend_developer"
          show-overflow-tooltip
          sortable
        ...
        <el-table-column
          ...
          label="后端开发"
          :width="defectColumnWidths.backend_developer"
          show-overflow-tooltip
          sortable
        ...
        <el-table-column
          label="模块路径"
          :width="defectColumnWidths.modules"
          sortable
        ...
        <el-table-column
          label="关联测试用例"
          :width="defectColumnWidths.related_testcases"
          sortable
        ...
        <el-table-column
          label="关联测试点"
          :width="defectColumnWidths.related_testpoints"
          sortable
        ...
        <el-table-column
          label="项目"
          :width="defectColumnWidths.project"
          sortable
        ...
        <el-table-column
          label="版本"
          :width="defectColumnWidths.version"
          sortable
        ...
        <el-table-column
          label="严重程度"
          :width="defectColumnWidths.severity"
          sortable
        ...
        <el-table-column
          label="状态"
          :width="defectColumnWidths.status"
          sortable
        ...
        <el-table-column
          label="处理人"
          :width="defectColumnWidths.assignees"
          sortable
        ...
        <el-table-column
          label="创建人"
          :width="defectColumnWidths.created_by"
          sortable
        ...
        <el-table-column
          ...
          label="附件数"
          :width="defectColumnWidths.attachments_count"
          align="center"
          sortable
...
import { deleteDefect, getDefects, importDefectExcel } from '@/api/defects'
...
import { buildActionColumnWidth, buildTableColumnWidth } from '@/utils/actionColumnWidth'
import { getUserDisplayName } from '@/utils/userDisplay'
import {
  buildDefectRelationRouteQuery,
  ensureUniqueDefectRelationItems,
} from '@/utils/defectRelations'
import { defectStatusOptions, getDefectStatusLabel, getDefectStatusTagType } from '@/utils/defectStatus'
...
const statusOptions = defectStatusOptions
...
const getStatusText = getDefectStatusLabel

const getStatusTagType = getDefectStatusTagType
...
const listStateQuery = computed(() => {
  ...
  const tab = props.embedded ? 'version-defects' : String(getQueryValue(route.query.tab) || '')
...
const buildDefectColumnWidth = (label, valueGetter, options = {}) => buildTableColumnWidth(
  label,
  defects.value,
  valueGetter,
  {
...
const defectColumnWidths = computed(() => ({
  priority: buildDefectColumnWidth('优先级', getDefectPriority, { min: 110, max: 128 }),
  problem_reason: buildDefectColumnWidth('问题原因', getDefectProblemReason, { min: 116, max: 280 }),
  root_cause: buildDefectColumnWidth('问题根因', getDefectRootCause, { min: 116, max: 280 }),
  requirement_id: buildDefectColumnWidth('需求编号', row => row?.requirement_id, { min: 128, max: 220 }),
  frontend_developer: buildDefectColumnWidth('前端开发', getDefectFrontendDeveloper, { min: 120, max: 200 }),
  backend_developer: buildDefectColumnWidth('后端开发', getDefectBackendDeveloper, { min: 120, max: 200 }),
  modules: buildDefectColumnWidth('模块路径', getDefectModuleLabels, { min: 128, max: 300 }),
  related_testcases: buildDefectColumnWidth('关联测试用例', getDefectTestcaseLabels, { min: 148, max: 300 }),
  related_testpoints: buildDefectColumnWidth('关联测试点', getDefectTestpointLabels, { min: 132, max: 300 }),
  project: buildDefectColumnWidth('项目', getDefectProjectName, { min: 96, max: 220 }),
  version: buildDefectColumnWidth('版本', getDefectVersionName, { min: 96, max: 220 }),
  severity: buildDefectColumnWidth('严重程度', row => getSeverityText(row?.severity), { min: 124, max: 144 }),
  status: buildDefectColumnWidth('状态', row => getStatusText(row?.status), { min: 104, max: 136 }),
  assignees: buildDefectColumnWidth('处理人', getDefectAssigneeNames, { min: 116, max: 260 }),
  created_by: buildDefectColumnWidth('创建人', getDefectCreatorName, { min: 104, max: 180 }),
  attachments_count: buildDefectColumnWidth('附件数', row => row?.attachments_count, { min: 108, max: 128 }),
}))
...
const loadDefects = async () => {
  ...
  try {
    ...
    const response = await getDefects(buildRequestParams())
...
const buildImportSummaryHtml = (payload = {}) => {
  const summaryLines = [`成功导入 ${Number(payload.created_count || 0)} 条缺陷`]
...
const handleImportFileChange = async (event) => {
  ...
  try {
    const response = await importDefectExcel({
      projectId: filters.projectId,
      versionId: filters.versionId,
      file,
    })
    await loadDefects()
    ElMessage.success('缺陷导入成功')
    ...
    ElMessage.error(errorMessage || '导入缺陷失败')
  } finally {
...
const goToEdit = (id) => {
  router.push({
    path: `/manual-testcases/defects/${id}/edit`,
    query: buildDefectRouteQuery(),
  })
}
...
const jumpToRelation = (item, defaultNodeType) => {
  ...
  query.from_tab = 'version-defects'
...
const handleViewMindmap = (row) => {
  ...
  if (!relationItem) {
    ElMessage.warning('当前缺陷未关联测试用例或测试点')
    return
  }
...
const handleDelete = async (row) => {
  try {
    await ElMessageBox.confirm(`确认删除缺陷“${row.code || row.title}”吗？`, '删除确认', {
...
.defect-table :deep(.el-table__header-wrapper .cell) {
  white-space: nowrap;
  word-break: keep-all;
}

.defect-table :deep(.el-table__body-wrapper .cell) {
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}

.defect-table :deep(.el-table__fixed-right .el-table__body-wrapper .cell) {
  overflow: visible;
}
