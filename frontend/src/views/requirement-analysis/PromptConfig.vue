<template>
  <div class="prompt-config">
    <div class="page-header">
      <h1>📝 Prompt</h1>
      <p>配置用于AI需求分析、需求评审、测试用例编写和用例评审的提示词</p>
    </div>
          ...
          <form @submit.prevent="saveConfig">
            ...
            <div class="form-group">
              ...
              <select v-model="configForm.prompt_type" class="form-select" required>
                ...
                <option value="requirement_writer">需求分析与编写提示词</option>
                <option value="requirement_reviewer">需求评审提示词</option>
                <option value="document_requirement_writer">需求文档创建需求提示词</option>
                <option value="document_testcase_writer">需求文档生成测试用例提示词</option>
              </select>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      previewConfig: {},
      promptTypeOptions: [
        { value: 'requirement_writer', label: '需求分析与编写提示词' },
        { value: 'requirement_reviewer', label: '需求评审提示词' },
        { value: 'document_requirement_writer', label: '需求文档创建需求提示词' },
        { value: 'document_testcase_writer', label: '需求文档生成测试用例提示词' },
        { value: 'writer', label: '用例编写提示词' },
        { value: 'reviewer', label: '用例评审提示词' }
      ],
      defaultPrompts: {
        requirement_writer: '',
        requirement_reviewer: '',
        document_requirement_writer: '',
        document_testcase_writer: '',
        writer: '',
        reviewer: ''
      },
      defaultPromptNames: {
  ...
  methods: {
    ...
    async loadDefaultPrompts() {
      ...
      try {
        ...
        this.defaultPromptNames = response.data.names || this.defaultPromptNames
    ...
    async confirmLoadDefaults() {
      ...
      try {
        await api.post('/requirement-analysis/api/prompts/ensure_defaults/')
    ...
    closeDefaultsModal() {
      ...
      this.activeTab = 'requirement_writer'
    },
...
.type-badge.requirement_writer {
  background: #e3f2fd;
  color: #1565c0;
}

.type-badge.requirement_reviewer {
  background: #fce4ec;
  color: #ad1457;
}
