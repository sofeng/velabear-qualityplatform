...
const pageOrderSaving = ref(false)
const elementOrderSaving = ref(false)
...
const pageTreeOptions = computed(() => {
  ...
  return buildTreeOptions(treeData.value)
})

const isTreeOrderSaving = computed(() => pageOrderSaving.value || elementOrderSaving.value)

const normalizeTreeStructure = (nodes = []) => {
  return nodes
    .filter(node => node.type === 'page')
    .map(node => {
      const children = Array.isArray(node.children) ? node.children : []
      const pageChildren = normalizeTreeStructure(children.filter(child => child.type === 'page'))
      ...
      return {
        ...node,
        children: [...pageChildren, ...elementChildren]
      }
    })
}

const buildPageOrderPayload = (nodes = [], parentId = null) => {
  return nodes
    .filter(node => node.type === 'page')
    .flatMap((node, index) => {
      const pageChildren = (node.children || []).filter(child => child.type === 'page')
      return [
        {
          id: node.id,
        ...
        ...buildPageOrderPayload(pageChildren, node.id)
      ]
    })
}

const buildElementOrderPayload = (nodes = []) => {
  return nodes.flatMap(node => {
    ...
    const children = Array.isArray(node.children) ? node.children : []
    const pageChildren = children.filter(child => child.type === 'page')
    const elementChildren = children.filter(child => child.type === 'element')

    return [
      ...buildElementOrderPayload(pageChildren),
...
const pageNodeContains = (node, targetId) => {
  if (!node || node.type !== 'page') return false
  if (node.id === targetId) return true

  return (node.children || [])
    .filter(child => child.type === 'page')
    .some(child => pageNodeContains(child, targetId))
}

const allowTreeNodeDrag = (node) => {
  if (isTreeOrderSaving.value) return false
  return ['page', 'element'].includes(node?.data?.type)
}

const allowTreeNodeDrop = (draggingNode, dropNode, type) => {
  if (isTreeOrderSaving.value) return false
  if (!['prev', 'inner', 'next'].includes(type)) return false

  const draggingType = draggingNode?.data?.type
  const dropNodeType = dropNode?.data?.type

  if (draggingType === 'page') {
    if (dropNodeType !== 'page') return false
    return !pageNodeContains(draggingNode.data, dropNode.data.id)
  }

  if (draggingType === 'element') {
    if (type === 'inner') {
      return dropNodeType === 'page'
    }
    return dropNodeType === 'element'
  }

  return false
}

const refreshTreeAfterOrderChange = async () => {
  await Promise.all([
    loadPages(),
    loadElementTree()
  ])
...
const handleTreeNodeDrop = async (draggingNode) => {
  if (!selectedProject.value || isTreeOrderSaving.value) return

  const draggedType = draggingNode?.data?.type
  if (!['page', 'element'].includes(draggedType)) {
    await refreshTreeAfterOrderChange()
    return
  }

  const normalizedTree = normalizeTreeStructure(treeData.value)
  treeData.value = normalizedTree

  try {
    if (draggedType === 'page') {
      pageOrderSaving.value = true
      await reorderElementGroups({
        project: selectedProject.value,
        groups: buildPageOrderPayload(normalizedTree)
      })
    } else {
      elementOrderSaving.value = true
      await reorderElements({
        project: selectedProject.value,
        elements: buildElementOrderPayload(normalizedTree)
      })
    }

    await refreshTreeAfterOrderChange()
    ElMessage.success(draggedType === 'page' ? '页面顺序已更新' : '元素顺序已更新')
  } catch (error) {
    console.error(draggedType === 'page' ? '保存页面顺序失败:' : '保存元素顺序失败:', error)
    ElMessage.error(draggedType === 'page' ? '页面顺序保存失败' : '元素顺序保存失败')
    await refreshTreeAfterOrderChange()
  } finally {
    pageOrderSaving.value = false
    elementOrderSaving.value = false
  }
}
