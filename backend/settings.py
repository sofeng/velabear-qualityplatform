...
def parse_env_flag(value, default=False):
    ...
    truthy_values = {'1', 'true', 'yes', 'y', 'on', 'debug', 'dev', 'development', 'local', 'test'}
...
DEBUG = parse_env_flag(config('DEBUG', default='True'), default=True)
...
LOCAL_APPS = [
    ...
    'apps.core',
    'apps.quality_analysis.apps.QualityAnalysisConfig',
...
ROOT_URLCONF = config('DJANGO_ROOT_URLCONF', default='backend.urls')
...
JWT_ROTATE_REFRESH_TOKENS = parse_env_flag(config('JWT_ROTATE_REFRESH_TOKENS', default='False'), default=False)
JWT_BLACKLIST_AFTER_ROTATION = (
    JWT_ROTATE_REFRESH_TOKENS
    and parse_env_flag(config('JWT_BLACKLIST_AFTER_ROTATION', default='False'), default=False)
)
...
SPECTACULAR_SETTINGS = {
    'TITLE': 'AIDevTestOps API',
    'DESCRIPTION': 'Test Case Management Platform API',
    'VERSION': '1.0.0',
    'SERVE_INCLUDE_SCHEMA': False,
}
ENABLE_API_DOCS = parse_env_flag(config('ENABLE_API_DOCS', default='True' if DEBUG else 'False'), default=DEBUG)
...
CELERY_TASK_ROUTES = {
    'apps.knowledge.tasks.index_knowledge_repository_task': {
        'queue': config('ASSET_CELERY_QUEUE', default='asset_insight'),
    },
}
...
EMAIL_VERIFICATION_EXPOSE_CODE = parse_env_flag(
    config('EMAIL_VERIFICATION_EXPOSE_CODE', default='True' if DEBUG else 'False'),
    default=DEBUG,
)
