...
class VersionListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        # 只显示用户有权限访问的项目的版本
        queryset = Version.objects.filter(projects__in=get_visible_projects()).distinct()
    ...
    def perform_create(self, serializer):
        ...
        # 检查项目权限
        accessible_projects = get_visible_projects()
        ...
        # 如果设置为默认版本，需要将其他版本的is_default设置为false
...
class VersionDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        return Version.objects.filter(projects__in=get_visible_projects()).distinct()
    ...
    def perform_update(self, serializer):
        ...
        if instance.is_default:
            # 获取该版本关联的所有项目
            project_ids = instance.projects.values_list('id', flat=True)
            # 将这些项目中其他版本的is_default设置为false
            Version.objects.filter(
                projects__id__in=project_ids
            ).exclude(id=instance.id).update(is_default=False)
