...
class BusinessRequirement(models.Model):
    ...
    CASE_GENERATION_STATUS_CHOICES = [
        ('not_generated', '未生成'),
        ('generating', '生成中'),
        ('generated', '已生成'),
        ('failed', '生成失败'),
    ...
    analysis = models.ForeignKey(RequirementAnalysis, on_delete=models.CASCADE, related_name='requirements', verbose_name='关联分析', null=True, blank=True)
    ...
    acceptance_criteria = models.TextField(verbose_name='验收标准', blank=True, default='')
    accepted_context = models.JSONField(verbose_name='已接受需求上下文', default=dict, blank=True)
    case_generation_status = models.CharField(max_length=20, choices=CASE_GENERATION_STATUS_CHOICES, default='not_generated', verbose_name='用例生成状态')
    audit_status = models.CharField(max_length=20, choices=AUDIT_STATUS_CHOICES, default='pending', verbose_name='审核状态')
    audited_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='audited_requirements', verbose_name='审核人')
...
class PromptConfig(models.Model):
    ...
    name = models.CharField(max_length=100, verbose_name='配置名称')
    prompt_type = models.CharField(max_length=50, choices=PROMPT_CHOICES, verbose_name='提示词类型')
...
class TestCaseGenerationTask(models.Model):
    ...
    output_mode = models.CharField(max_length=10, choices=OUTPUT_MODE_CHOICES, default='batch', verbose_name='输出模式')
    stream_buffer = models.TextField(blank=True, default='', verbose_name='流式输出缓存')
    stream_position = models.IntegerField(default=0, verbose_name='流式输出位置')
    last_stream_update = models.DateTimeField(null=True, blank=True, verbose_name='最近一次流式更新时间')
...
class AIModelService:
    ...
    @staticmethod
    async def call_openai_compatible_api(config: AIModelConfig, messages: List[Dict[str, str]], max_tokens: int = None) -> Dict[str, Any]:
        """调用OpenAI兼容格式的API。max_tokens 可按调用覆盖模型配置默认值（用于大结构化输出，如业务逻辑9视图）。"""
        headers = {
