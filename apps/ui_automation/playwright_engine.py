...
from playwright.async_api import async_playwright, Page, Browser, BrowserContext, Frame, TimeoutError as PlaywrightTimeout
import logging
...
class PlaywrightTestEngine:
    ...
    def _reset_frame_context(self):
        """重置 iframe 上下文，回到主文档。"""
        self.current_frame = None
        self.frame_stack = []

    def _get_current_context(self):
        """获取当前操作上下文，优先使用当前 iframe。"""
        return self.current_frame or self.page

    def _build_locator(self, context, locator_strategy: str, locator_value: str):
        """根据定位策略在当前上下文中构造 Locator。"""
        strategy = (locator_strategy or 'css').lower()

        if strategy == 'id':
            return context.locator(f'#{locator_value}')
        if strategy in ['css', 'css selector']:
            if any(keyword in locator_value.lower() for keyword in ['dropdown', 'el-select', ':has(', 'li']):
                if 'visible=true' not in locator_value:
                    return context.locator(f"{locator_value} >> visible=true").first
                return context.locator(locator_value).first
            return context.locator(locator_value)
        if strategy == 'xpath':
            if any(keyword in locator_value.lower() for keyword in ['dropdown', 'el-select', ':has(', 'li']):
                if 'visible=true' not in locator_value:
                    return context.locator(f"xpath={locator_value} >> visible=true").first
                return context.locator(f"xpath={locator_value}").first
            if '[' in locator_value and ']' in locator_value:
                return context.locator(f'xpath={locator_value}')
            return context.locator(f'xpath={locator_value}').first
        if strategy == 'text':
            return context.get_by_text(locator_value)
        if strategy == 'name':
            return context.locator(f'[name="{locator_value}"]')
        if strategy == 'placeholder':
            return context.get_by_placeholder(locator_value)
        if strategy == 'role':
            return context.get_by_role(locator_value)
        if strategy == 'label':
            return context.get_by_label(locator_value)
        if strategy == 'title':
            return context.get_by_title(locator_value)
        if strategy == 'test-id':
            return context.get_by_test_id(locator_value)
        return context.locator(locator_value)
    ...
    async def execute_step(self, step, element_data: Dict) -> Tuple[bool, str, Optional[str]]:
        ...
        element_name = element_data.get('name', '页面')
        ...
        if element_wait_timeout is not None and element_wait_timeout > 0:
            timeout_ms = element_wait_timeout * 1000
        elif step.wait_time:
        ...
        try:
            ...
            # 其他操作需要元素定位器
            if action_type == 'enterIframe':
                current_context = self._get_current_context()
                iframe_locator = self._build_locator(current_context, locator_strategy, locator_value)
                await iframe_locator.wait_for(state='attached', timeout=timeout_ms)
                iframe_handle = await iframe_locator.element_handle(timeout=timeout_ms)
                ...
                self.frame_stack.append(target_frame)
                self.current_frame = target_frame

                execution_time = round(time.time() - start_time, 2)
                log = f"✓ 进入iframe '{element_name}' 成功\n"
                log += f"  - 定位器: {locator_strategy}={locator_value}\n"
                log += f"  - 超时设置: {timeout_ms/1000}秒\n"
                log += f"  - 当前iframe层级: {len(self.frame_stack)}\n"
                log += f"  - 执行时间: {execution_time}秒"
                return True, log, None

            elif action_type == 'exitIframe':
                if self.frame_stack:
                    self.frame_stack.pop()
                self.current_frame = self.frame_stack[-1] if self.frame_stack else None

                execution_time = round(time.time() - start_time, 2)
                log = "✓ 退出iframe成功\n"
                if self.frame_stack:
                    log += f"  - 已返回上一级iframe，当前层级: {len(self.frame_stack)}\n"
                else:
                    log += "  - 已返回主文档\n"
                log += f"  - 执行时间: {execution_time}秒"
                return True, log, None
            ...
            current_context = self._get_current_context()
            locator = self._build_locator(current_context, locator_strategy, locator_value)
            ...
            if action_type == 'click':
                ...
                if is_select_trigger:
                    ...
                    try:
                        ...
                        try:
                            await asyncio.sleep(0.5)
                            dropdown = current_context.locator('.el-select-dropdown').first
                            if await dropdown.is_visible():
                                logger.info(f"多选下拉框未自动关闭，点击空白处关闭...")
                                await current_context.click('body', position={'x': 10, 'y': 10}, timeout=3000)
                                auto_close_msg = " + 自动关闭"
                            else:
                                auto_close_msg = ""
                        except:
