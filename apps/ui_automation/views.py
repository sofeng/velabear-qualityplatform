...
class ElementViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).prefetch_related('script_usages__script').order_by('group_id', 'order', 'name')

    def _get_next_order(self, project_id, group_id=None, exclude_id=None):
    ...
    def perform_create(self, serializer):
        ...
        if group_id is not None and order is None:
            save_kwargs['order'] = self._get_next_order(project_id=project_id, group_id=group_id)
    ...
    def perform_update(self, serializer):
        ...
        next_group_id = serializer.validated_data.get('group_id', instance.group_id)
        project_id = serializer.validated_data.get('project_id', instance.project_id)
        group_changed = 'group_id' in serializer.validated_data and next_group_id != instance.group_id
        order_provided = 'order' in serializer.validated_data

        save_kwargs = {}
        if group_changed and not order_provided:
            save_kwargs['order'] = self._get_next_order(
                project_id=project_id,
    ...
    def reorder(self, request):
        ...
        if not project_id:
            return Response({'error': '缺少项目ID'}, status=status.HTTP_400_BAD_REQUEST)
        if not isinstance(elements_data, list) or not elements_data:
            return Response({'error': '缺少排序数据'}, status=status.HTTP_400_BAD_REQUEST)

        project_elements = self.get_queryset().filter(project_id=project_id)
        elements_by_id = {element.id: element for element in project_elements}
        requested_ids = [item.get('id') for item in elements_data]

        if any(element_id is None for element_id in requested_ids):
            return Response({'error': '排序数据中存在无效元素ID'}, status=status.HTTP_400_BAD_REQUEST)
        if len(set(requested_ids)) != len(requested_ids):
            return Response({'error': '排序数据中存在重复元素ID'}, status=status.HTTP_400_BAD_REQUEST)
        if any(element_id not in elements_by_id for element_id in requested_ids):
            return Response({'error': '排序数据中包含无权限访问的元素'}, status=status.HTTP_400_BAD_REQUEST)

        project_groups = ElementGroup.objects.filter(project_id=project_id)
        groups_by_id = {group.id: group for group in project_groups}

        with transaction.atomic():
            for item in elements_data:
                element = elements_by_id[item['id']]
                group_id = item.get('group_id')
                order = item.get('order')

                if group_id is None or group_id not in groups_by_id:
                    return Response({'error': '目标页面无效'}, status=status.HTTP_400_BAD_REQUEST)
                if not isinstance(order, int):
                    return Response({'error': '排序值必须为整数'}, status=status.HTTP_400_BAD_REQUEST)

                target_group = groups_by_id[group_id]
                update_fields = ['group', 'page', 'order', 'updated_at']

                if element.group_id != group_id and element.parent_element_id is not None:
                    element.parent_element = None
                    update_fields.append('parent_element')

                element.group_id = group_id
                element.page = target_group.name
                element.order = order
                element.save(update_fields=update_fields)

        return Response({'message': '元素顺序更新成功'})
...
class ElementGroupViewSet(viewsets.ModelViewSet):
    ...
    def _get_next_order(self, project_id, parent_group_id=None, exclude_id=None):
        queryset = self.get_queryset().filter(
            project_id=project_id,
    ...
    def perform_create(self, serializer):
        parent_group = serializer.validated_data.get('parent_group')
    ...
    def perform_update(self, serializer):
        instance = serializer.instance
        next_parent = serializer.validated_data.get('parent_group', instance.parent_group)
        next_project = serializer.validated_data.get('project', instance.project)
        parent_changed = 'parent_group' in serializer.validated_data and next_parent != instance.parent_group
        order_provided = 'order' in serializer.validated_data

        save_kwargs = {}
        if parent_changed and not order_provided:
            save_kwargs['order'] = self._get_next_order(
                project_id=next_project.id,
                parent_group_id=next_parent.id if next_parent else None,
                exclude_id=instance.id
            )
    ...
    def reorder(self, request):
        ...
        project_groups = self.get_queryset().filter(project_id=project_id)
        groups_by_id = {group.id: group for group in project_groups}
        requested_ids = [item.get('id') for item in groups_data]

        if any(group_id is None for group_id in requested_ids):
            return Response({'error': '排序数据中存在无效页面ID'}, status=status.HTTP_400_BAD_REQUEST)
        if len(set(requested_ids)) != len(requested_ids):
            return Response({'error': '排序数据中存在重复页面ID'}, status=status.HTTP_400_BAD_REQUEST)
        if any(group_id not in groups_by_id for group_id in requested_ids):
            return Response({'error': '排序数据中包含无权限访问的页面'}, status=status.HTTP_400_BAD_REQUEST)

        parent_map = {group.id: group.parent_group_id for group in project_groups}

        for item in groups_data:
            group_id = item.get('id')
            parent_group_id = item.get('parent_group')
            order = item.get('order')

            if not isinstance(order, int):
                return Response({'error': '排序值必须为整数'}, status=status.HTTP_400_BAD_REQUEST)
            if parent_group_id is not None and parent_group_id not in groups_by_id:
                return Response({'error': '目标父页面无效'}, status=status.HTTP_400_BAD_REQUEST)
            if parent_group_id == group_id:
                return Response({'error': '页面不能拖拽到自身下方'}, status=status.HTTP_400_BAD_REQUEST)

            parent_map[group_id] = parent_group_id
        ...
        for group_id in requested_ids:
            ...
            while current_id is not None:
                if current_id in visited:
                    return Response({'error': '页面层级存在循环引用'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        with transaction.atomic():
            for item in groups_data:
                ...
                group.parent_group_id = item.get('parent_group')
                group.order = item['order']
                group.save(update_fields=['parent_group', 'order', 'updated_at'])

        return Response({'message': '页面顺序更新成功'})
...
class TestCaseViewSet(viewsets.ModelViewSet):
    ...
    def _generate_step_log(self, step, step_result='success'):
        ...
        elif step.action_type == 'enterIframe':
            log_parts.append(f"进入iframe '{element_name}'")
            log_parts.append(f"- 使用定位器: {locator_info}")
            if step_result == 'success':
                log_parts.append(f"- iframe 切换成功 - 耗时 {execution_time}s")
            else:
                log_parts.append(f"- iframe 切换失败 - 目标 iframe 未找到或尚未加载完成")

        elif step.action_type == 'exitIframe':
            log_parts.append("退出当前iframe")
            if step_result == 'success':
                log_parts.append(f"- 已返回上一级上下文 - 耗时 {execution_time}s")
            else:
                log_parts.append("- 退出 iframe 失败")
        ...
        try:
                ...
                if bots:
                    for bot in bots:
                        ...
                        if bot.get('enabled', True) and bot.get('enable_ui_automation', True):
                            ...
                            logger.info(f"AI军火库通知机器人 {bot.get('name')} 未启用UI自动化测试，跳过")
            ...
            for bot in all_webhook_bots:
                if not bot.get('enabled', True):
                    logger.info(f"跳过未启用的机器人: {bot.get('name', 'Unknown')}")
                    continue

                bot_type = bot.get('type', 'unknown')
                connection_mode = bot.get('connection_mode') or 'webhook'
                webhook_url = bot.get('webhook_url') or ''
                if connection_mode != 'auth' and not webhook_url:
                    logger.info(f"跳过未配置Webhook URL的机器人: {bot.get('name', 'Unknown')}")
                    continue
...
浏览器: {task.browser.capitalize()}"""
                ...
                if connection_mode == 'auth':
                    try:
                        from apps.core.notification_connections import sanitize_bot_for_log, send_authorized_notification

                        title = f"UI自动化定时任务执行{status_text}"
                        send_result = send_authorized_notification(
                            bot,
                            title=title,
                            markdown_text=detail_content,
                        )
                        UiNotificationLog.objects.create(
                            ...
                            recipient_info=[{'name': bot.get('name', 'Unknown'), 'connection_mode': 'auth'}],
                            webhook_bot_info=sanitize_bot_for_log(bot),
                            notification_content=json.dumps(send_result.get('request_payload') or {}, ensure_ascii=False),
                            status='success' if send_result.get('ok') else 'failed',
                            error_message='' if send_result.get('ok') else send_result.get('detail', ''),
                            response_info=send_result.get('response_info') or {},
                            sent_at=timezone.now()
                        )
                        if send_result.get('ok'):
                            logger.info(f"授权通知发送成功 - {bot_type}")
                        else:
                            logger.error(f"授权通知发送失败 - {bot_type}: {send_result.get('detail')}")
                    except Exception as e:
                        logger.error(f"授权通知发送失败 - {bot_type}: {str(e)}", exc_info=True)
                    continue
